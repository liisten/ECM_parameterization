% output a OCV for a given SOC value using the charge OCV curve
...
function ocv = OCVfromSOCtemp_CHG(soc,temp,model)
if ~isscalar(temp)
...
OCV = model.(['T',num2str(temp)]).OCVc; % the charge OCV
...
%     ocv = chgOCV(idx(1))*(1-p1) + chgOCV(idx(2))*(1-p2);
% end
